...
successful subset은 하나의 target digest, 하나의 page diff, 하나의 QuePaxa slot, 하나의
qlog entry를 공유한다. apply 시 control sidecar의 한 `FULL` transaction이 applied tip과 모든
receipt를 같은 anchor로 zero-or-all commit한다.
...
checkpoint/close한다. canonical DB를 같은 filesystem의 staging file로 복사하고, speculative
connection은 `synchronous=OFF`로 실행한다. 이 설정은 staging scratch에만 적용되며 durability
ACK의 기준이 아니다. 결정 전 canonical DB, qlog, control state는 바뀌지 않는다.

staging transaction을 commit한 뒤 checkpoint, integrity check와 close를 끝내야 proposal이
될 수 있다. `diff_closed_databases`는 closed target을 완전히 한 번 scan하면서 changed page와
`target_db_digest`를 동시에 계산한다. 별도의 preparation-time target digest 재scan은 없다.
이 fused scan은 I/O 최적화일 뿐 full-file coverage와 canonical effect bytes를 바꾸지 않는다.
proposal 준비 중 canonical tip이 바뀌거나 다른 payload가 slot을 차지하면 staging은
폐기하거나 foreign winner의 새 base에 맞춰 재준비한다.

## 7. durable apply와 crash recovery
...
3. control sidecar에 entry anchor, base/target digest의 pending intent를 기록하고 fsync한다.
4. base를 새 temp file로 복제해 after-image와 target length를 적용한다.
5. temp digest, SQLite header와 integrity를 검증하고 temp file을 fsync한다.
6. prepared target을 재사용할 때는 owned file identity와 inode, size, digest를 다시 확인한다.
7. authoritative path로 atomic rename하고 parent directory를 fsync한다.
8. 한 control transaction에서 applied tip, target digest, ordered receipt subset을 같은 anchor로
   commit하고 pending intent를 지운다.
9. canonical DB를 reopen하고 serving gate를 연다.

preparation의 fused digest scan은 이 apply 계약을 줄이지 않는다. apply-time base/target digest,
temp/target sync, owned-inode와 symlink 방어, atomic rename, parent-directory sync, pending intent,
receipt/control `FULL` commit은 모두 유지한다.
