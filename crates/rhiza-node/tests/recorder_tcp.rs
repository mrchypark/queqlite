...
async fn three_tcp_recorders_commit_and_expose_record_and_proof() {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        assert!(inspector.inspect_decision_proof(1).unwrap().is_none());
        inspector
            .install_decision_proof(proof, &install_membership)
            .unwrap();
        assert!(inspector.inspect_decision_proof(1).unwrap().is_some());
    })
    .await
    .unwrap();
