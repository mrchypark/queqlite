...
fn open_rejects_the_incompatible_prototype_request_schema() {
    ...
    Connection::open(&path)
        .unwrap()
        .execute_batch(
            "DROP TABLE __rhiza_requests;
             CREATE TABLE __rhiza_requests (
    ...
    match SqliteStateMachine::open_existing(&path) {
        Err(Error::Sqlite(message)) => assert_eq!(
            message,
            "incompatible __rhiza_requests schema; recreate this prototype database"
        ),
...
fn open_migrates_the_v1_request_table_without_losing_decided_requests() {
    ...
    Connection::open(&path)
        ...
        .execute_batch(&format!(
            "DROP TABLE __rhiza_requests;
             ...
             INSERT INTO __rhiza_requests VALUES (
                 'request-1', 7, x'{}', x'{}'
             );
             UPDATE __rhiza_meta SET value = x'31' WHERE key = 'schema_version';",
    ...
    let schema_version: String = inspection
        .query_row(
            "SELECT CAST(value AS TEXT) FROM __rhiza_meta WHERE key = 'schema_version'",
            [],
            |row| row.get(0),
        )
        .unwrap();
...
fn noop_only_advances_progress_and_unsupported_payloads_are_rejected() {
    ...
    let raw_sql = v1_entry(3, EntryType::Command, noop.hash, b"DELETE FROM __rhiza_kv");
...
fn legacy_stopped_configuration_state_fails_closed() {
    ...
    conn.execute(
        "UPDATE __rhiza_meta SET value = ?1 WHERE key = 'configuration_state'",
        params![legacy_stopped],
    )
    .unwrap();
    conn.execute(
        "UPDATE __rhiza_meta SET value = x'32' WHERE key = 'schema_version'",
        [],
    )
    .unwrap();
    drop(conn);
...
fn scalar_config_database_migrates_to_active_zero_digest() {
    ...
    Connection::open(&path)
        .unwrap()
        .execute(
            "DELETE FROM __rhiza_meta WHERE key = 'configuration_state'",
            [],
        )
        .unwrap();
...
fn qsql_v2_persists_statement_results_and_returns_them_on_exact_retries() {
    ...
    let result_blob: Vec<u8> = Connection::open(&path)
        .unwrap()
        .query_row(
            "SELECT result_blob FROM __rhiza_requests WHERE request_id = 'result-1'",
            [],
            |row| row.get(0),
        )
        .unwrap();
...
fn qsql_v2_retry_fails_closed_when_the_stored_result_is_not_canonical() {
    ...
    Connection::open(&path)
        .unwrap()
        .execute(
            "UPDATE __rhiza_requests SET result_blob = x'00' WHERE request_id = 'corrupt-result'",
            [],
        )
        .unwrap();
...
fn sql_boundary_rejects_nondeterminism_internal_state_and_wrong_statement_mode() {
    ...
    for (index, statements) in [
        ...
        vec![
            ...
            "ALTER TABLE ordinary RENAME TO __rhiza_hidden",
        ],
        vec![
            "CREATE TABLE parents(id INTEGER PRIMARY KEY)",
