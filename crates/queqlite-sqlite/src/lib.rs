...
CREATE TABLE IF NOT EXISTS __queqlite_meta (
    key TEXT PRIMARY KEY,
    value BLOB NOT NULL
);

CREATE TABLE IF NOT EXISTS __queqlite_migrations (
...
CREATE TABLE IF NOT EXISTS __queqlite_requests (
...
CREATE TABLE IF NOT EXISTS __queqlite_kv (
    key TEXT PRIMARY KEY,
    value TEXT NOT NULL
);
"#;
...
const SQL_EXECUTOR_POLICY_VERSION: &str = "queqlite-sql-policy-v2";
...
impl SqliteStateMachine {
    ...
    fn migrate_request_results(&self) -> Result<()> {
        ...
        match schema_version {
            ...
            LEGACY_SCHEMA_VERSION => {
                ...
                tx.execute(
                    "ALTER TABLE __queqlite_requests ADD COLUMN result_blob BLOB",
                    [],
                )
                .map_err(sqlite_error)?;
                put_meta(
    ...
    pub fn get_value(&self, key: &str) -> Result<Option<String>> {
        self.conn
            .query_row(
                "SELECT value FROM __queqlite_kv WHERE key = ?1",
                params![key],
                |row| row.get(0),
            )
            .optional()
        ...
        .prepare(
            "SELECT name FROM sqlite_schema
             WHERE type = 'table'
               ...
               AND lower(name) NOT GLOB '__queqlite_*'
             ORDER BY name",
        )
        .map_err(sqlite_error)?;
...
fn record_request_digest(
    ...
    conn.execute(
        "INSERT INTO __queqlite_requests(
             request_id,
             original_log_index,
             original_log_hash,
             command_digest,
...
fn reserved_name(name: &str) -> bool {
    name.to_ascii_lowercase().starts_with("__queqlite_")
}
...
fn validate_reserved_schema(conn: &Connection) -> Result<()> {
    let unexpected: Option<String> = conn
        .query_row(
            "SELECT name
             FROM sqlite_schema
             WHERE lower(name) GLOB '__queqlite_*'
               AND name NOT IN (
                   '__queqlite_meta',
                   '__queqlite_migrations',
                   '__queqlite_requests',
                   '__queqlite_kv'
               )
             LIMIT 1",
    ...
    if let Some(name) = unexpected {
        return Err(Error::InvalidCommand(format!(
            "SQL object uses reserved Queqlite namespace: {name}"
        )));
    }
    Ok(())
}
...
fn put_value(conn: &Connection, key: &str, value: &str) -> Result<()> {
    conn.execute(
        "INSERT INTO __queqlite_kv(key, value)
         VALUES (?1, ?2)
...
fn validate_initialized(conn: &Connection) -> Result<()> {
    ...
    conn.prepare("SELECT key, value FROM __queqlite_kv LIMIT 0")
        .map_err(sqlite_error)?;
...
fn validate_requests_schema(conn: &Connection, schema_version: u64) -> Result<()> {
    ...
    Err(Error::Sqlite(
        "incompatible __queqlite_requests schema; recreate this prototype database".into(),
    ))
}
...
fn put_meta(conn: &Connection, key: MetaKey, value: &[u8]) -> Result<()> {
    conn.execute(
        "INSERT INTO __queqlite_meta(key, value)
         VALUES (?1, ?2)
         ON CONFLICT(key) DO UPDATE SET value = excluded.value",
